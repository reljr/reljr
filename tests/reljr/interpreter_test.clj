...
(t/deftest RAInterpreterSelection
  (t/is (= (i/select table-one #(= (:R/a %) 2))
           #{#:R{:a 2, :b 5, :c 1}}))
  (t/is (= (i/select table-two #(not= (:R/a %) 154))
           #{#:R{:a 2234, :b 35465, :d 3845} #:R{:a 133, :b 7345, :d 234}}))
  (t/is (= (i/select table-three #(s/starts-with? (:Customers/name %) "H"))
           #{#:Customers{:name "Henry", :city "Miami", :budget 20000}})))

(t/deftest RARenameColumn
  (t/is (= (i/rename-column table-one :R/a :R/thelettera)
           #{#:R{:b 5, :c 1, :thelettera 2} #:R{:b 7, :c 2, :thelettera 1}
             #:R{:b 2, :c 3, :thelettera 1}}))
  (t/is (= (i/rename-column table-three :Customers/budget :Customers/money)
...
(t/deftest RACrossProduct
  ...
  (t/is (= (i/cross-product table-one table-three)
...
(t/deftest RAInnerJoin
  (t/is (= (i/inner-join table-one table-four #(= (:R/a %) (:S/a %2)))
