...
(t/deftest RAInterpreterProjection
  (t/is (= (table/project table-one [:R/a]) #{#:R{:a 2} #:R{:a 1}}))
  (t/is (= (table/project table-two [:R/d]) #{#:R{:d 234} #:R{:d 43} #:R{:d 3845}}))
  (t/is (= (table/project table-three [:Customers/name]) #{#:Customers{:name "Joanne"} #:Customers{:name "Henry"}})))

(t/deftest RAInterpreterSelection
  (t/is (= (table/select table-one #(= (:R/a %) 2))
           #{#:R{:a 2, :b 5, :c 1}}))
  (t/is (= (table/select table-two #(not= (:R/a %) 154))
           #{#:R{:a 2234, :b 35465, :d 3845} #:R{:a 133, :b 7345, :d 234}}))
  (t/is (= (table/select table-three #(s/starts-with? (:Customers/name %) "H"))
           #{#:Customers{:name "Henry", :city "Miami", :budget 20000}})))

(t/deftest RARenameColumn
  (t/is (= (table/rename-column table-one :R/a :R/thelettera)
           #{#:R{:b 5, :c 1, :thelettera 2} #:R{:b 7, :c 2, :thelettera 1}
             #:R{:b 2, :c 3, :thelettera 1}}))
  (t/is (= (table/rename-column table-three :Customers/budget :Customers/money)
...
(t/deftest RARenameRelation
  (t/is (= (table/rename table-two "NewTable")
           #{#:NewTable{:a 2234, :b 35465, :d 3845} #:NewTable{:a 133, :b 7345, :d 234}
             #:NewTable{:a 154, :b 3452, :d 43}})))

(t/deftest RAOrderBy
  (t/is (= (into [] (table/order-records-by table-one [[:R/c <]]))
           [#:R{:a 2, :b 5, :c 1} #:R{:a 1, :b 7, :c 2} #:R{:a 1, :b 2, :c 3}]))
  (t/is (= (into [] (table/order-records-by table-three [[:Customers/budget >]]))
           [#:Customers{:name "Joanne", :city "New York City", :budget 80000}
            #:Customers{:name "Henry", :city "Miami", :budget 20000}]))
  (t/is (= (into [] (table/order-records-by table-two [[:R/b <]]))
           [#:R{:a 154, :b 3452, :d 43} #:R{:a 133, :b 7345, :d 234}
            #:R{:a 2234, :b 35465, :d 3845}])))

(t/deftest RACrossProduct
  (t/is (= (table/cross-product table-one table-two)
  ...
  (t/is (= (table/cross-product table-one table-three)
...
(t/deftest RAInnerJoin
  (t/is (= (table/inner-join table-one table-four #(= (:R/a %) (:S/a %2)))
